...
String pathway, Groove, Sting, Baby, FeelTheFiyaaaah, XOTourLlif3, FamilyTies;
int currentSong=2;
...
void setupMusic() {
 ...
 songs[2] = minim.loadFile( pathway + FeelTheFiyaaaah );
 songs[3] = minim.loadFile( pathway + XOTourLlif3 ) ;
 songs[4] = minim.loadFile( pathway + FamilyTies ) ;
...
void drawMusic() {
  ...
  println( "/tEnd Of Song:", songs[currentSong].length() );
  //
  autoPlayMusic();
  //
}//End drawMusic
...
void keyPressedMusic() {
  ...
  //Forward
  if ( key == 'e' || key == 'E' ) {
    //Skips forward to the end of song
